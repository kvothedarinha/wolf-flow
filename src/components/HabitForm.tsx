...
export function HabitForm({ open, onOpenChange, habit }: HabitFormProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (kind === "build" && frequency === "daily" && weekdays.length === 0)
    ...
    save.mutate(
      {
        ...
        color,
        kind,
        frequency: kind === "quit" ? "daily" : frequency,
      ...
      <DialogContent className="max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label>Tipo</Label>
            <Tabs value={kind} onValueChange={(v) => setKind(v as "build" | "quit")}>
              <TabsList className="grid grid-cols-2 w-full">
                <TabsTrigger value="build">Construir</TabsTrigger>
                <TabsTrigger value="quit">Largar</TabsTrigger>
              </TabsList>
            </Tabs>
            {kind === "quit" && (
              <p className="text-xs text-muted-foreground">
                O sucesso é não fazer: registre apenas as recaídas e proteja seus dias livres.
              </p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder={kind === "quit" ? "Ex.: Não fumar" : "Ex.: Beber 2L de água"}
              autoFocus
            />
          </div>
